...
from conode.application.errors import OfferNotFoundError
...
from conode.domain.offer.model import (
    Offer,
    OfferContext,
    OfferGroup,
    OfferId,
...
class OfferRepositoryImpl(OfferRepository):
    ...
    async def create(self, offer: Offer) -> None:
        logger.debug("Repository create offer", offer_id=offer.id)
    ...
    async def update(self, offer: Offer) -> None:
        logger.debug("Repository update offer", offer_id=offer.id)
        await self.session.execute(
            update(Offer)
            .where(
    ...
    async def get_by_id(self, offer_id: OfferId) -> Offer:
        logger.debug("Repository get offer by id", offer_id=offer_id)
        ...
        logger.debug("Repository fetched offer by id", found=offer is not None)
...
class OfferLinkRepositoryImpl(OfferLinkRepository):
    ...
    async def create(self, offer_link: OfferLink) -> None:
        logger.debug("Repository create offer link", offer_link_id=offer_link.id)
        await self.session.execute(
    ...
    async def update(self, offer_link: OfferLink) -> None:
        logger.debug("Repository update offer link", offer_link_id=offer_link.id)
        await self.session.execute(
            update(OfferLink)
            .where(
